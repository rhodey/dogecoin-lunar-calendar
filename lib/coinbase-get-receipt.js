...
function getAddresses(account) {
  let base = '/v2'
  let path = `${base}/accounts/${account.id}/addresses`
  return new Promise((res, rej) => {
    http('GET', path).then((result) => {
      let stat = result[0]
      if (stat !== 200) {
...
function getAddressId(account, address) {
  return getAddresses(account).then((addresses) => {
...
function getTransactions(accountId, addressId) {
  let base = '/v2'
  let path = `${base}/accounts/${accountId}/addresses/${addressId}/transactions`
  return new Promise((res, rej) => {
    http('GET', path).then((result) => {
      let stat = result[0]
      if (stat !== 200) {
...
function getReceiveTotals(accountId, addressId) {
  return getTransactions(accountId, addressId).then((txns) => {
    let receipts = txns.filter((txn) => txn.from !== undefined)
    let totalUsd = receipts.reduce((acc, txn) => acc += txn.native_amount.amount, 0.0)
    let totalCoin = receipts.reduce((acc, txn) => acc += txn.amount.amount, 0.0)
    return [totalUsd, totalCoin]
  })
}

if (argv._.length < 2) { onError(new Error('bad cmd arguments')) }
let coin = argv._[0]
let addressId = argv._[1]
let accountId = ACCOUNTS[coin]

return getReceiveTotals(accountId, addressId).then((totals) => {
